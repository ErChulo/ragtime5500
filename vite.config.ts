  ...
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      html = html
        ...
        // Use function replacers: the bundled worker source contains literal "import { readFileSync } from 'node:fs';
import { createRequire } from 'node:module';
import { basename } from 'node:path';
import { defineConfig, type Plugin } from 'vite';
import react from '@vitejs/plugin-react';
  ...
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      html = html
        ...
        .replace(styleDirective, `${styleDirective} 'nonce-${INLINE_NONCE}'`)
        "
        // sequences, which String.replace replacement strings interpret specially.
